...
export function useNovenaProgress(novenaId) {
    const getInitialProgress = () => {
        const saved = localStorage.getItem(`novena_progress_${novenaId}`);
        return saved ? JSON.parse(saved) : [];
    };
    const [completedDays, setCompletedDays] = useState(getInitialProgress);
    const [syncStatus, setSyncStatus] = useState('idle');
    const isInitialMount = useRef(true);
    const serverDaysRef = useRef([]);
    useEffect(() => {
    ...
    const toggleDay = useCallback((day) => {
        setCompletedDays((prev) => prev.includes(day) ? prev.filter(d => d !== day) : [...prev, day].sort());
    }, []);
